...
import { createLogger, format, transport, type Logger, LogEntry, transports } from 'winston';
...
const logger: Logger = createLogger({
  format: combine(label({ label: environment.env }), timestamp({ format: LOG_DATE_FORMAT }), json(), prettyPrint({ colorize: true }), logFormattter),
...
export const logWithoutConsole = (logEntry: LogEntry) => {
  ...
  const fileTransport = logger.transports.find((transport) => transport instanceof transports.File);
