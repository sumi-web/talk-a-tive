...
export const HR = (color: ChalkColor = 'white', char: string = '-', length: number = 60): string => {
...
export const envFileNotFoundError = (key: CommonEnvKeys): string => {
  ...
  const fileNotFoundMessage = `${chalk.greenBright(defaultEnvFile)} or ${chalk.greenBright(envFile)} is required`;
  return `
    \r${divider}\n
